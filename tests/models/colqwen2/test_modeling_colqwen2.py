...
class ColQwen2VisionText2TextModelTester:
    def __init__(
        self,
        parent,
        batch_size=3,
    ...
    def create_and_check_colqwen2_model_fp16_forward(
        self, config, input_ids, pixel_values, attention_mask, image_grid_thw
    ):
        model = ColQwen2ForConditionalGeneration(config=config)
    ...
    def create_and_check_colqwen2_model_fp16_autocast_forward(
        self, config, input_ids, pixel_values, attention_mask, image_grid_thw
    ):
...
@require_torch
class ColQwen2ModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    """
    Model tester for `ColQwen2ForConditionalGeneration`.
    """

    all_model_classes = (ColQwen2ForConditionalGeneration,) if is_torch_available() else ()
    all_generative_model_classes = (ColQwen2ForConditionalGeneration,) if is_torch_available() else ()
    test_pruning = False
    test_head_masking = False
    _is_composite = True

    def setUp(self):
        self.model_tester = ColQwen2VisionText2TextModelTester(self)
    ...
    @unittest.skip(reason="Compile not yet supported because in ColQwen2 models")
...
@require_torch
class ColQwen2IntegrationTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_small_model_integration_test(self):
        model = ColQwen2ForConditionalGeneration.from_pretrained(

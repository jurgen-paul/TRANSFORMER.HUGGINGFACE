...
from transformers.models.colqwen2.image_processing_colqwen2 import smart_resize
...
if is_vision_available():
    ...
    from transformers import ColQwen2ImageProcessor


class ColQwen2ImageProcessingTester:
    def __init__(
        self,
        parent,
        batch_size=7,
...
@require_torch
@require_vision
class ColQwen2ImageProcessingTest(ImageProcessingTestMixin, unittest.TestCase):
    image_processing_class = ColQwen2ImageProcessor if is_vision_available() else None

    def setUp(self):
        super().setUp()
        self.image_processor_tester = ColQwen2ImageProcessingTester(self)
    ...
    @unittest.skip(reason="ColQwen2ImageProcessor doesn't treat 4 channel PIL and numpy consistently yet")
    ...
    def test_video_inputs(self):
        ...
        for num_frames, expected_dims in expected_dims_by_frames.items():
            image_processor_tester = ColQwen2ImageProcessingTester(self, num_frames=num_frames)
    ...
    def test_custom_patch_size(self):
        ...
        for patch_size in (1, 3, 5, 7):
            image_processor_tester = ColQwen2ImageProcessingTester(self, patch_size=patch_size)
            video_inputs = image_processor_tester.prepare_video_inputs(equal_resolution=True)
            prcocess_out = image_processing(None, videos=video_inputs, return_tensors="pt")
            encoded_video = prcocess_out.pixel_values_videos
